...
import com.example.movieproject.ui.viewmodel.VisibilityViewModel

@Composable
fun VisibilitiesScreen(navController: NavController) {
    val viewModel: VisibilityViewModel = viewModel()
    val visibilities by viewModel.visibilities.collectAsState()
    ...
    Box(
        ...
        if (visibilities.isEmpty()) {
            Column(
                ...
                Text(
                    text = stringResource(R.string.no_visibilities),
                    color = Color.White,
            ...
            Column(
                ...
                LazyColumn(
                    ...
                    items(visibilities) { movie ->
                        MovieItem(
                            movie = movie,
                             isFavorite = favorites.any { it.id == movie.id },
                            onMovieClick = { clickedMovie ->
                                navController.navigate(
                                    ScreenRoutes.VisibilitiesScreen.createRoute(clickedMovie.id)
                                )
                            },
                            onFavoriteClick = { clickedMovie ->
                                viewModel.toggleMovie(clickedMovie)
                            },
                        )
                    }
                }
                ...
                if (visibilities.isNotEmpty()) {
                    Button(
                        onClick = { viewModel.clearAllVisibilities() },
                        colors = ButtonDefaults.buttonColors(
                        ...
                        Text(
                            text = stringResource(R.string.clearVis),
                            color = Color.White
                        )
                    }
                }
